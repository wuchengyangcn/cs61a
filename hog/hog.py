...
def tail_points(opponent_score):
    ...
    if (tens>=ones):
        return (2*(tens-ones)+1)
    else:
        return (2*(ones-tens)+1)
    # END PROBLEM 2
...
def perfect_square(score):
    """
    Check whether the score is a square
    """
    root = 0
    while (root*root<score):
        root+=1
...
def next_perfect_square(score):
    """
    Return the next square number given the current score
    """
    root = 0
    while (root*root<=score):
        root+=1
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def f(current_score, opponent_score):
        return n
    return f
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while (current<100):
        opponent=0
        while (opponent<100):
            if (strategy(current,opponent)!=base):
                return False
            opponent+=1
        current+=1
    return True
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if (square_update(0,score,opponent_score)>=GOAL): return 0
    elif (GOAL-score<=roll_dice(1)): return 1
    elif (GOAL-score<=roll_dice(2)): return 2
    averaged_dice=make_averaged(roll_dice,total_samples=100)
