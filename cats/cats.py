...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if (current_diff<best_diff) or ((current_diff<=limit) and (""==best_word)):
            best_word=word
...
def feline_fixes(typed, source, limit):
    ...
    if (typed[0]!=source[0]):
        ...
        offset = 0
    return offset+feline_fixes(typed[1:], source[1:], limit-offset)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if (limit<0):  # Fill in the condition
        ...
        if (start[0]!=goal[0]):
            ...
            offset=0
        substitute = offset+minimum_mewtations(start[1:],goal[1:],limit-offset)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    index=0
    while (index<len(typed) and typed[index]==prompt[index]):
        index+=1
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match,j,i)<time(match,fastest,i):
                fastest=j
        results[fastest].append(get_word(match,i))
    return results
    # END PROBLEM 10
